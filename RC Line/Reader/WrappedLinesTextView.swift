...
final class SpeechHighlightDelegate: NSObject, ObservableObject, AVSpeechSynthesizerDelegate {
    ...
    func prepare(lineIndex: Int, utteranceText: String) {
...
extension WrappedLinesTextView {
    ...
    func lineRow(
                ...
                speechHighlightedText(line: line, index: index)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 10)
                            .fill(
                                colorScheme == .dark
                                ? storedHighlightDark.color : storedHighlight.color
                            ).opacity(
                                (index == focusingLine && !wasScrolled) ? selectedTextOpacity : 0)
                    )
                    ...
                    .overlay {
                        if index == focusingLine && !wasScrolled {
                            RoundedRectangle(cornerRadius: 10)
                                .stroke(
                                    colorScheme == .dark
                                    ? storedHighlightDark.color.opacity(borderOpacity)
                                    : storedHighlight.color.opacity(borderOpacity), lineWidth: 2
                                )
                        }
                    }
                    .blur(
        ...
        .alert(
            "Feature is not available", isPresented: $showSimulatorAlert,
            actions: {
    ...
    private func speechHighlightedText(line: String, index: Int) -> Text {
        ...
        guard speechHighlightDelegate.lineIndex == index,
              ...
              let highlightRange = Range(characterRange, in: speechHighlightDelegate.utteranceText),
              speechHighlightDelegate.utteranceText == line
        else {
        ...
        attributed.append(dimmedAttributedString(String(line[..<highlightRange.lowerBound]), color: dimColor))
        attributed.append(dimmedAttributedString(String(line[highlightRange]), color: baseColor))
        attributed.append(dimmedAttributedString(String(line[highlightRange.upperBound...]), color: dimColor))
        return Text(attributed)
    }
    ...
    private func startSpeaking(text: [String], index: Int, scrollProxy: ScrollViewProxy) {
        ...
        Task {
            ...
            while focusingLine < text.count && speaking {
                ...
                let utteranceText = sentence
                let utterance = AVSpeechUtterance(string: utteranceText)
                speechHighlightDelegate.prepare(lineIndex: i, utteranceText: utteranceText)
